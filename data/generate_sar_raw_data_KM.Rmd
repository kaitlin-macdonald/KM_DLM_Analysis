...
Outflow <- read.csv(here( "LWG_Outflow.csv")) %>% mutate(month_date=format(as.Date(paste0(mm.dd,'-',year), "%m-%d-%Y"))) %>% mutate(month=month(month_date), Year=year(month_date))
...
Temperature <- read.csv(here( "LWG_Temp.csv")) %>% mutate(month_date=format(as.Date(paste0(mm.dd,'-',year), "%m-%d-%Y"))) %>% mutate(month=month(month_date), Year=year(month_date)) %>% filter(!is.na(value))
...
```{r PNI_data}
PNI <- read.csv(here( "pni_data.csv")) %>% select(Year, Annual.PNI) %>% rename(year=Year)
```
...
sar_raw_data <- SalmonSurvCUI %>% 
 ...
 # rename("value" = CUI.apr) %>% 
  ...
  left_join(df_cui, by = c("year")) %>% 
  select(-CUI.apr) %>% 
        add_row(year = max(.$year)+1, logit.s = NA, sar.method = "Scheuerell and Williams (2005)", Apr = NA, Sep=NA, Oct=NA)   #add 2006 NA row for ahead forecast
...
load(here( "LGRLGA_allpassage_wild_spsu_chinook.rda"))
...
df_cjs_raw<-read_csv(here( "lgr.lga.surv.yr.1994.2021_250309.csv")) 
...
df.cjs.LGRLGA.apr<-df.cjs %>% inner_join(df_cui_apr, by = "year")  %>% inner_join(df_cuti_apr, by = "year") %>% inner_join(df_beuti_apr, by = "year")  %>% inner_join(df_mldno3_apr, by = "year")  %>% inner_join(df_outflow_apr, by = "year")  %>% inner_join(df_temp_apr, by = "year")  %>% inner_join(PNI, by = "year") %>% inner_join(df_cui_mean, by = "year") %>% inner_join(df_cuti_mean, by = "year")  %>% inner_join(df_beuti_mean, by = "year")  %>% inner_join(df_mldno3_mean, by = "year")

#Save LGRLGA CJS data
saveRDS(df.cjs.LGRLGA.apr, "df_cjs_LGRLGA_apr.rds")
```
