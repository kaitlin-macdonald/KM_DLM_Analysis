...
Outflow <- read.csv(here( "data/LWG_Outflow.csv")) %>% mutate(month_date=format(as.Date(paste0(mm.dd,'-',year), "%m-%d-%Y"))) %>% mutate(month=month(month_date), Year=year(month_date))

df_outflow<- Outflow %>% group_by(month, Year) %>% summarise(mean.out=mean(value))

df_outflow_apr<- df_outflow %>% filter(month==4) %>% rename(year=Year) 

ggplot(df_outflow %>% filter(month==4), aes(x = Year, y = mean.out)) +
  geom_line() +
  geom_point() +
  labs(title = "Outflow",
       x = "Year",
...
Temperature <- read.csv(here( "data/LWG_Temp.csv")) %>% mutate(month_date=format(as.Date(paste0(mm.dd,'-',year), "%m-%d-%Y"))) %>% mutate(month=month(month_date), Year=year(month_date)) %>% filter(!is.na(value))

#Get monthly means and filter out March because daily means not available for all of march
df_temp<- Temperature %>% group_by(month, Year) %>% summarise(mean.temp=mean(value)) %>% filter(month!=3)

df_temp_apr<- df_temp %>% filter(month==4) %>% rename(year=Year)

ggplot(df_temp %>% filter(month==4), aes(x = Year, y = mean.temp)) +
  geom_line() +
  geom_point() +
...
#CBR SAR + April data
df.cbr.LGRLGA.apr<-df.cbr %>% inner_join(df_cui_apr, by = "year")  %>% inner_join(df_cuti_apr, by = "year") %>% inner_join(df_beuti_apr, by = "year")  %>% inner_join(df_mldno3_apr, by = "year")  %>% inner_join(df_outflow_apr, by = "year")  %>% inner_join(df_temp_apr, by = "year")  %>% inner_join(PNI, by = "year")  %>% inner_join(df_cui_mean, by = "year") %>% inner_join(df_cuti_mean, by = "year")  %>% inner_join(df_beuti_mean, by = "year")  %>% inner_join(df_mldno3_mean, by = "year")
...
df_SAR_LGRLGA_MV<-sar_raw_data %>% full_join(df.cbr, by="year") %>%  full_join(df.cjs, by="year") %>% left_join(df_cui_apr, by = "year")  %>% left_join(df_cuti_apr, by = "year") %>% left_join(df_beuti_apr, by = "year")  %>% left_join(df_mldno3_apr, by = "year")  %>% left_join(df_outflow_apr, by = "year")  %>% left_join(df_temp_apr, by = "year")  %>% left_join(PNI, by = "year") %>% left_join(df_cui_mean, by = "year") %>% left_join(df_cuti_mean, by = "year")  %>% left_join(df_beuti_mean, by = "year")  %>% left_join(df_mldno3_mean, by = "year") %>% left_join(SST_pre1212, by = "year") %>% left_join(SST_entry567, by = "year") %>% left_join(SST_6mo, by = "year") %>% 
  select(-c(month.x, month.y, month))
