#NEed to figure out Error in dlmLL(logit.s_cjs, dlmModPoly(1) + dlmModReg(X = SST_pre_z, addInt = FALSE)) : 
#error code -4 from Lapack routine dgesdd

#Need to figure out how to get same LL as MARSS
...
loglik_cjs_ref <- dlmLL(logit.s_cjs, mod_cjs_ref)
...
#dlmLL caculates the neg. LL (lower is better)
loglik_cjs_CUI <- dlmLL(logit.s_cjs, mod_cjs_CUI)
...
buildFun_cjs_CUTI<- function(parm){
  ...
  mod_CUTI$m0 <- rep(10, 9)
  mod_CUTI$C0 <- diag(9) * 10
  return(mod_CUTI)
  
}

fit_cjs_CUTI<- dlmMLE(logit.s_cjs, parm = rep(10,9), build=buildFun_cjs_CUTI, hessian=TRUE)
...
loglik_cjs_mnCUI <- dlmLL(logit.s_cjs, mod_cjs_mnCUI)
...
loglik_cjs_PNI <- dlmLL(logit.s_cjs, mod_cjs_PNI)
...
loglik_cjs_SST_pre <- dlmLL(logit.s_cjs, mod_cjs_SST_pre)
...
loglik_cjs_SST_entry <- dlmLL(logit.s_cjs, mod_cjs_SST_entry)
...
loglik_cjs_SST_6mo <- dlmLL(logit.s_cjs, mod_cjs_SST_6mo)
